...
# Backgrounds
main_menu_bg = load_image("\\resources\\background\\bg.png")
sc_bg = load_image("\\resources\\background\\bg1.png")
spa_bg = load_image("\\resources\\background\\bg1.png")
dff_bg = load_image("\\resources\\background\\bg1.png")
dbf_bg = load_image("\\resources\\background\\bg1.png")
...
class Static_Best_Fit(Static_First_Fit):
    ...
    def create_result_windows( self, all_job_sizes, all_job_arrival_time, all_job_run_time, all_partition_sizes):
        ...
        def find_best_fit_partition(size):
            best_fit_index = -1
            for i in range(len(partition_sizes)):
                if int(partition_sizes[i]) >= int(size):
                    if best_fit_index == -1:
                        best_fit_index = i
                    elif self.partitionState[best_fit_index]:
                        if self.partitionState[i]:
                            temp_prev_time = self.partition_history[best_fit_index][-1]
                            temp_time = self.partition_history[i][-1]
                            if (int(partition_sizes[i]) < int(partition_sizes[best_fit_index])) and (temp_time <= temp_prev_time):
                                best_fit_index = i
                        else:
                            if (int(partition_sizes[i]) < int(partition_sizes[best_fit_index])):
                                best_fit_index = i
                    elif (int(partition_sizes[i]) < int(partition_sizes[best_fit_index])):
                        best_fit_index = i
        ...
        for i in range(5):
            ...
            if best_fit_index != -1:
                if self.partitionState[best_fit_index]:
...
class Static_Worst_Fit(Static_First_Fit):
    ...
    def create_result_windows( self, all_job_sizes, all_job_arrival_time, all_job_run_time, all_partition_sizes):
        ...
        job_times = deepcopy(all_job_arrival_time)
        job_times = [datetime.datetime.strptime(time, '%H:%M') for time in job_times]

        job_waiting_list = [0, 1, 2, 3, 4]

        def find_worst_fit_partition(size):
            worst_fit_index = -1
            for i in range(len(partition_sizes)):
                if int(partition_sizes[i]) >= int(size) and self.partitionState[i] == False:
                    if worst_fit_index == -1:
                        worst_fit_index = i
                    elif (int(partition_sizes[i]) >= int(partition_sizes[worst_fit_index])):
                        worst_fit_index = i
            return worst_fit_index

        while job_times:
            job_times = list(set(job_times))
            job_times.sort()
            current_time = job_times[0]
            print("Current Time: ", current_time)

            for i in range(len(self.partitionState)):
                if self.partitionState[i] == True:
                    temp_time = self.partition_history[i][-1]
                    if (temp_time >= current_time):
                        self.partitionState[i] = False
                        print("Partition Finish Time: ", temp_time)

            temp_waiting_jobs = deepcopy(job_waiting_list)
            for i in temp_waiting_jobs:
                worst_fit_index = find_worst_fit_partition(job_sizes[i])
                print(i, worst_fit_index)
                if worst_fit_index != -1:
                    self.partitionState[worst_fit_index] = True
                    self.job_state[i] = True

                    if len(self.partition_history[worst_fit_index]) == 0:
                        self.temp_start_time = datetime.datetime.strptime(self.all_job_arrival_time[i], '%H:%M')
                    else:
                        self.temp_start_time = self.partition_history[worst_fit_index][-1]
                    ...
                    self.temp_finish_time = (self.temp_start_time - self.time_zero + (datetime.datetime.strptime(self.all_job_run_time[i], '%H:%M')))
                    self.all_finish_time.append(self.temp_finish_time.time())
                    job_times.append(self.temp_finish_time)

                    self.partition_history[worst_fit_index].append(self.temp_finish_time)
                    self.all_job_results[i].append(self.temp_finish_time)

                    self.all_job_results[i].append(self.temp_cpu_wait)
                    self.all_job_results[i].append(worst_fit_index)
                    self.all_job_results[i].append(i)
                    self.all_job_results[i].append("After")

                    print("job processed: ", i, worst_fit_index)
                    print("current time: ", current_time )

                    job_waiting_list.remove(i)
            if not job_times:
                break
            else:
                job_times.pop(0)
